...
(function ($) {
  ...
  function animateRing(score, circ, color) {
    ...
    $({ val: circ }).animate(
      { val: targetOffset },
      {
        ...
        easing: "swing",
        step() {
          $circle.css("stroke-dashoffset", this.val);
        },
      },
    );
    $({ n: 0 }).animate(
      { n: score },
      {
        duration: 1100,
        step() {
          $("#sapc-ring-val").text(Math.round(this.n));
        },
      },
    );
  }
  ...
  function animateCounter($el, target, duration = 900) {
    $({ n: 0 }).animate(
      { n: target },
      {
        duration,
        step() {
          $el.text(Math.round(this.n).toLocaleString());
        },
        done() {
  ...
  function pulseRingGlow(color) {
    ...
    $({ v: 0 }).animate(
      { v: 8 },
      {
        ...
        step() {
          $wrap.css({
            filter: `drop-shadow(0 0 ${this.v}px ${color})`,
          });
        },
      },
    );
  ...
  function filterKeywords(tab) {
    ...
    if (!rows.length) {
      $ul.append(
        `<li class="sapc-no-data">${tab === "all" ? "No audited posts yet — run a full audit." : `No ${tab} found with SEO data.`}</li>`,
      );
      return;
    }
  ...
  function renderReadability(rows) {
    ...
    rows.forEach((r) => {
      ...
      const low =
        r.status === "bad" ? '<span class="sapc-read-low">Low</span>' : "";
      const $li = $(`
  ...
  function renderSchema(types) {
    ...
    types.forEach((s) => {
      $g.append(
        `<div class="sapc-schema-item"><span class="sapc-schema-name">${esc(s.type)}</span><span class="sapc-dot sapc-dot-${s.status}"></span></div>`,
      );
    });
  }
  ...
  function renderMonitor(rows, redirects, pending) {
    ...
    if (rows && rows.length) {
      rows.forEach((b) => {
        ...
        const badge =
          hits > 0
            ? `<span class="sapc-monitor-hits-inline">${hits} hits</span>`
            : "";
        $ul.append(
          `<li class="sapc-monitor-row"><div class="sapc-monitor-label"><strong>${esc(b.label)}</strong>${badge}</div><div class="sapc-monitor-url"><code>${esc(b.url)}</code></div></li>`,
        );
      });
    } else {
      $ul.append(
  ...
  function openIssueModal(issueKey) {
    ...
    $.post(AJAX_URL, {
      ...
      .done((res) => {
        ...
        if (!posts.length) {
          $list.append(
            '<li class="sapc-no-data">No affected pages found — great work!</li>',
          );
        } else {
          posts.forEach((p) => {
            const actions = [
              p.edit_url
                ? `<a href="${esc(p.edit_url)}" class="sapc-mpost-btn" target="_blank">Edit</a>`
                : "",
              p.view_url
                ? `<a href="${esc(p.view_url)}" class="sapc-mpost-btn sapc-mpost-view" target="_blank">View</a>`
                : "",
            ].join("");
            const note = p.note
              ? `<span class="sapc-mpost-note">${esc(p.note)}</span>`
              : "";
            const type = p.type
              ? `<span class="sapc-kw-type-badge sapc-type-${esc(p.type)}">${esc(p.type)}</span>`
              : "";
            $list.append(`
  ...
  function applyData(d) {
    ...
    animateCounter(
      $("#sapc-stat-critical"),
      parseInt(d.critical_issues, 10) || 0,
    );
    animateCounter($("#sapc-stat-indexed"), parseInt(d.indexed_pages, 10) || 0);
    animateCounter(
      $("#sapc-stat-readability"),
      parseInt(d.avg_readability, 10) || 0,
    );
    ...
    $("#sapc-og-title").html(
      "&#x2713; " +
        (parseInt(d.og_title_posts, 10) || 0).toLocaleString() +
        " posts",
    );

    if (parseInt(d.og_img_missing, 10) > 0) {
      $("#sapc-og-img")
        .attr("class", "sapc-og-miss")
        .html("&#x2717; " + d.og_img_missing + " missing");
    } else {
  ...
  $(document).on(
    "click",
    "#sapc-modal-close, #sapc-modal-close-btn",
    closeModal,
  );
  ...
  $(function () {
    ...
    $(
      "#sapc-stat-audited, #sapc-stat-critical, #sapc-stat-indexed, #sapc-stat-readability",
    ).each(function () {
      const $el = $(this);
